                ...
                <div class="w-full sm:w-3/4 lg:w-1/2 bg-white rounded-lg shadow-lg p-6">
                    <form action="{{ route('dishes.update', $dish->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea required name="description" id="description" class="form-input" value="{{ $dish->description }}"
                                cols="60">{{ $dish->description }}</textarea>
                        </div>

                        <div class="mb-4">
                            <label for="price"
                                class="block text-gray-700 font-bold mb-2">{{ __('Prezzo (€)') }}</label>
                            <input type="number" required name="price" id="price" class="form-input"
                                value="{{ $dish->price }}">
                        </div>
